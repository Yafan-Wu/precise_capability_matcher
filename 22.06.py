...
class SolverEngine:
    ...
    def build_capabilities(self, cap_data):
        ...
        for res, entries in cap_data.items():
            for e in entries:
                ...
                # Keep all capabilities, don't skip any
                cid = f"{res}::{name}"
    ...
    def determine_transport_mode(self, step, step_idx):
        """Determine transport mode for a step (Feed, Discharge, Transfer)"""
        sid = step['ID']
        
        # 0. Spezielle Standortbehandlung (höchste Priorität)
        if step_idx == 0:  # Der erste Schritt im Prozess ist immer Feed
            return 'feed'
        if step_idx == len(self.step_ids) - 1:  # Der letzte Schritt des Prozesses ist immer die discharge
            return 'discharge'
            
        step_type = step.get('SemanticDescription', '').split('#')[-1].lower()
        
        # 1. Prüfen, ob es sich um einen Transportschritt handelt
        if step_type not in self.TRANSPORT_TYPES:
            return None  # Not a transport step
        
        # 2. Prüfen, ob explizit angegebene Modusparameter vorhanden sind.
        ...
        # 5. Materialfluss analysieren
        if 'DirectedLinks' in self.rec_data:
            # Find inputs and outputs for this step
            inputs = [link for link in self.rec_data['DirectedLinks'] if link['ToID'] == sid]
            outputs = [link for link in self.rec_data['DirectedLinks'] if link['FromID'] == sid]
            
            # Check material sources and destinations
            input_resources = set()
            output_resources = set()
            
            # Get input material sources
            for link in inputs:
                # Find source step
                source_steps = [s for s in self.rec_data['ProcessElements'] if s['ID'] == link['FromID']]
                if source_steps:
                    source_step = source_steps[0]
                    # If source is a processing step, record its resource
                    if any(cap for cap in self.cap_list if cap['id'].startswith("resource:") and source_step['ID'] in cap['id']):
                        input_resources.add(source_step['ID'].split(':')[0])
            
            # Get output material destinations
            for link in outputs:
                # Find target step
                target_steps = [s for s in self.rec_data['ProcessElements'] if s['ID'] == link['ToID']]
                if target_steps:
                    target_step = target_steps[0]
                    # If target is a processing step, record its resource
                    if any(cap for cap in self.cap_list if cap['id'].startswith("resource:") and target_step['ID'] in cap['id']):
                        output_resources.add(target_step['ID'].split(':')[0])
            
            # Determine mode based on material flow
            if input_resources and not output_resources:  # Only input, no output to other device
                return 'feed'
            elif not input_resources and output_resources:  # Only output, no input from other device
                return 'discharge'
            elif input_resources and output_resources:  # Connecting different devices
                return 'transfer'
        
        # 6. Default value - ensure all steps are classified
        return 'transfer'  # Most common mode
    ...
    def build_constraints(self):
        ...
        for step_idx, step in enumerate(self.rec_data.get('ProcessElements', [])):
            sid = step.get('ID', '')
            # Determine transport mode for each step
        ...
        for step_idx, step in enumerate(self.rec_data.get('ProcessElements', [])):
            ...
            for cap in self.cap_list:
                cid = cap['id']
                # Check if capability contains all required parameters
                missing = [k for k in params if k not in cap['props']]
                if missing:
                    invalid[cid] = [f"Missing parameter: {m}" for m in missing]
                    continue
                reasons = []
                ...
                for k, vstr in params.items():
                    ...
                    if lo is not None and c_hi is not None and c_hi < lo:
                        reasons.append(f"{k}: Max value {c_hi} < requirement {lo}")
                    if hi is not None and c_lo is not None and c_lo > hi:
                        reasons.append(f"{k}: Min value {c_lo} > requirement {hi}")
                if reasons:
                    invalid[cid] = reasons
                else:
                    valid.append(cid)
        ...
        for step_idx, step in enumerate(self.rec_data.get('ProcessElements', [])):
            ...
            if step_type in self.TRANSPORT_TYPES:
...
class SMTGuiApp:
    ...
    def export_master_recipe(self):
        """Export B2MML XML master recipe"""
        ...
        for i, step in enumerate(rd.get("ProcessElements", [])):
            ...
            # 确保所有步骤都有运输模式定义
            transport_mode = step.get('_transport_mode', 'process')
            
            if transport_mode == 'feed':
                elem_type = "Feed"
                desc = f"{step_id} Feed operation"
                # 添加物料标识
                material = ET.SubElement(re_el, "b2mml:MaterialID")
                material.text = "H2O"  # 默认水
            elif transport_mode == 'discharge':
                elem_type = "Discharge"
                desc = f"{step_id} Discharge operation"
            elif transport_mode == 'transfer':
                elem_type = "Transfer"
                desc = f"{step_id} Transfer operation"
            else:
                elem_type = "Process"
                desc = f"{step_id} Processing operation"
                ...
                for (step_id, cap_id), var in self.engine.step_vars.items():
                    if self.engine.model.evaluate(var) and step_equipment_map.get(step_id) == equip:
                        ...
                        if cap_name not in capabilities_added:
                            ...
                            # Determine process type based on capability
                            if 'feed' in cap_name.lower():
                                epe_type = "FeedProcess"
                            elif 'discharge' in cap_name.lower():
                                epe_type = "DischargeProcess"
                            elif 'transfer' in cap_name.lower() or 'dosing' in cap_name.lower():
                                epe_type = "TransferProcess"
                            else:
                                epe_type = "Process"
